...
void assert_str(
        const char* cmp, const char* exp, const char* real, const char* caller,
        int line);
...
void assert_wstr(
        const char* cmp, const wchar_t* exp, const wchar_t* real,
...
void assert_data(
        const unsigned char* exp, size_t expsize, const unsigned char* real,
...
void assert_compare(
        const char* cmp, intmax_t exp, intmax_t real, const char* caller,
        int line);
...
void assert_compare_u(
        const char* cmp, uintmax_t exp, uintmax_t real, const char* caller,
        int line);
...
void assert_interval(
        intmax_t exp1, intmax_t exp2, intmax_t real, const char* caller,
        int line);
...
void assert_dbl_compare(
        const char* cmp, double exp, double real, double tol,
        const char* caller, int line);
...
void assert_str(
    ...
    if ((!exp ^ !real)
        ...
        CTEST_ERR(
                "%s:%d  assertion failed, '%s' %s '%s'", caller, line, exp, cmp,
                real);
    }
}
...
void assert_wstr(
    ...
    if ((!exp ^ !real)
        ...
        CTEST_ERR(
                "%s:%d  assertion failed, '%ls' %s '%ls'", caller, line, exp,
                cmp, real);
    }
}
...
void assert_data(
    ...
    if (expsize != realsize) {
        CTEST_ERR(
                "%s:%d  expected %" PRIuMAX " bytes, got %" PRIuMAX, caller,
...
void assert_compare(
    ...
    if (!get_compare_result(cmp, c3, c3 == 0)) {
        CTEST_ERR(
                "%s:%d  assertion failed, %" PRIdMAX " %s %" PRIdMAX "", caller,
                line, exp, cmp, real);
    }
}
...
void assert_compare_u(
    ...
    if (!get_compare_result(cmp, c3, c3 == 0)) {
        CTEST_ERR(
                "%s:%d  assertion failed, %" PRIuMAX " %s %" PRIuMAX, caller,
                line, exp, cmp, real);
    }
}
